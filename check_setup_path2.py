...
def check_setup_path():
    ...
    if len(lines) > 59:
        ...
        if expected_filename in line_60:
            print(f"CHECK: Filename {expected_filename} is present in the path")
        else:
            print(f"WARNING: Filename {expected_filename} is not present in the path")
        ...
        if os.path.exists(actual_file):
            print(f"CHECK: Actual file exists: {actual_file}")
        else:
            print(f"WARNING: Actual file does not exist: {actual_file}")
