...
def _find_key_file() -> Path | None:
    candidates = [
        Path(__file__).parent / "data" / ".arch_key",
        Path(__file__).parent / ".arch_key",
        Path.cwd() / "data" / ".arch_key",
        Path.cwd() / ".arch_key",
    ]
    if getattr(sys, "frozen", False):
        candidates.insert(0, Path(sys.executable).parent / "data" / ".arch_key")
        candidates.insert(1, Path(sys.executable).parent / ".arch_key")
    for c in candidates:
        if c.exists():
            return c
    return None
...
def _get_secret(secret_arg: str | None) -> str:
    ...
    print("Error: no --secret provided and .arch_key not found.", file=sys.stderr)
